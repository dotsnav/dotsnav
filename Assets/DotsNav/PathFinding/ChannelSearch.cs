...
namespace DotsNav.PathFinding
{
    readonly unsafe struct ChannelSearch
    {
        ...
        public PathQueryState Search(float2 start, float2 goal, Navmesh navmesh, List<Gate> path, float radius, DynamicBuffer<TriangleElement> triangleIds, out int cost)
        {
            var open = _open;
            var closed = _closed;
            var steps = _steps;
